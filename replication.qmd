...
│   ├── llm_scoring.py                # v3: full-corpus LLM quality validation
...
│   │
│   ├── sroi_clean_dataset_v3.csv     # ← USE THIS (LLM-validated quality scores)
│   ├── sroi_clean_dataset_v2.csv     # v2 (keyword scores + confidence metadata)
│   ├── sroi_ratios_subset.csv        # 64 reports with SROI ratios
│   ├── sroi_factors.csv              # 11 calculation elements × 383 reports
│   │
│   ├── llm_scoring_log.csv           # ← 3,064 LLM coding decisions with key phrases
│   ├── llm_scoring_summary.md        # ← Keyword vs LLM comparison by principle
│   ├── audit_report.md               # ← Full data quality audit narrative
│   ├── audit_flags.csv               # ← Per-report classification flags
│   └── classification_evidence.csv   # ← Keyword evidence per report
├── validation/
│   ├── validate_coding.py            # Multi-agent IRR framework
│   ├── sample_manifest.csv           # Stratified validation sample (n=60)
│   ├── agent_A_codings.csv           # Systematic coder results
│   ├── agent_B_codings.csv           # Critical coder (false positive detection)
│   ├── reconciled_codings.csv        # GPT-4o reconciler on disagreements
│   ├── false_positive_log.csv        # Detected keyword false positives
│   └── reliability_report.md         # IRR statistics (κ, α) by principle
...
# Step 2: LLM quality validation — full corpus (requires OPENAI_API_KEY)
export OPENAI_API_KEY=sk-...
python data/llm_scoring.py   # ~10 min, resumable via cache

# Step 3: Main analysis
python data/analysis.py

# Step 4: Simulations (bootstrap, Monte Carlo)
python data/simulations.py

# Step 5: Calculation elements
python data/factors_analysis.py

# Step 6: Compile website
quarto render
```
...
|--------|------|-------|
| `data_audit.py` | ~2 min | |
| `llm_scoring.py` | ~10 min | Resumable; uses `.llm_score_cache.json` |
| `analysis.py` | ~30 sec | |
...
# Load v3 (LLM-validated)
df = pd.read_csv('data/sroi_clean_dataset_v3.csv')

# Main results — dual measure
print(f"Keyword compliance: {df['quality_pct_kw'].mean():.1f}%")   # ~40.4%
print(f"LLM compliance:     {df['quality_pct_llm'].mean():.1f}%")  # ~18.6%

# R1: LLM compliance by principle
for p in ['p1','p2','p3','p4','p5','p6','p7','p8']:
    print(f"{p}: kw={df[f'{p}_involve_stakeholders' if p=='p1' else p].mean():.3f}, llm={df[f'{p}_involve_stakeholders_llm' if p=='p1' else p+'_llm' if p+'_llm' in df.columns else p].mean():.3f}")

# R2: High-confidence sectors only
print(df[df['sector_confidence']=='HIGH']['quality_pct_llm'].mean())
...
| File | Description |
|------|-------------|
| [sroi_clean_dataset_v3.csv](sroi_clean_dataset_v3.csv) | **Main dataset (recommended)** — LLM-validated quality scores |
| [sroi_clean_dataset_v2.csv](sroi_clean_dataset_v2.csv) | v2 — keyword scores + confidence metadata |
| [llm_scoring_log.csv](llm_scoring_log.csv) | 3,064 LLM coding decisions with key phrases and justifications |
| [audit_flags.csv](audit_flags.csv) | Per-report classification flags |
| [classification_evidence.csv](classification_evidence.csv) | Keyword quality score evidence |
| [sroi_factors.csv](sroi_factors.csv) | Calculation elements |
| [llm_scoring.py](llm_scoring.py) | LLM full-corpus scoring script |
...
| [validation/validate_coding.py](validate_coding.py) | Multi-agent IRR framework |
| [validation/reliability_report.md](reliability_report.md) | IRR statistics (κ, α) by principle |
